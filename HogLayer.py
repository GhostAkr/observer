...
def experiment(number, image, layer, cuda):
    for i in range(number):
        getHog(image, layer, cuda)
...
if __name__ == '__main__':
    ...
    layer = HOGLayer(nbins=12, pool=2)
    if cuda:
        layer = layer.cuda()

    # y2 = getHog(im, layer, cuda)
    cProfile.run('experiment(1000, im, layer, cuda)')
    exit()
    print(y2.shape)
    for bin in range(hog.nbins):
        im = y2[0, bin]
